 ...
 *
 * Реализует рекурсивный парсер выражений по грамматике
 ...
 *   unary   = (+|-) unary | postfix
 *   postfix = primary { ! }
 *   primary = число | (expr) | pi | e
 *           | sqrt(expr) | ln(expr) | log(expr)
 *
 ...
 *   pi, e             константы
 *
 * Класс инкапсулирует состояние парсера ($src — исходная строка,
 * $pos — текущая позиция). Это пример инкапсуляции из ООП (лаба 6):
 * внешний код не может ни прочитать, ни изменить эти поля напрямую,
 * только через публичный метод evaluate().
 */
class Calculator
{
    /** Разбираемая в данный момент строка-выражение. */
    private string $src = '';

    /** Текущая позиция парсера в $src. */
    private int $pos = 0;

    /**
     * Главный метод: разобрать и вычислить выражение.
     * Возвращает результат в виде строки, готовой к отображению.
     * Бросает Exception при ошибке (деление на ноль, бяка в синтаксисе и т.п.).
     */
    public function evaluate(string $expression): string
    {
        ...
        // Базовая проверка: непустое выражение, только разрешённые символы,
        // сбалансированы скобки.
        $this->validate($expression);

        // Запускаем рекурсивный спуск с самого верхнего правила грамматики.
        $result = $this->expr();

        // После разбора в строке не должно остаться "хвоста".
        if ($this->pos !== strlen($this->src)) {
            throw new Exception("лишние символы в конце");
        }

        // Красивое форматирование дробных чисел: убираем лишние нули в конце.
        if (is_float($result)) {
